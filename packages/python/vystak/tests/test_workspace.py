...
class TestWorkspace:
    ...
    def test_sandbox_with_capabilities(self):
        ws = Workspace(name="dev", type=WorkspaceType.SANDBOX, filesystem=True, terminal=True, timeout="30m")
        assert ws.filesystem is True
        assert ws.terminal is True
        assert ws.timeout == "30m"

    def test_persistent_with_path(self):
        ws = Workspace(name="research", type=WorkspaceType.PERSISTENT, persist=True, path="research/{agent}/", max_size="100mb")
    ...
    def test_mounted_with_provider(self):
        ...
        ws = Workspace(name="docs", type=WorkspaceType.MOUNTED, provider=provider, path="/shared/invoices/")

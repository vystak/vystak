...
class TestResolveEnvFile:
    def test_default_env(self, tmp_path):
        (tmp_path / "vystak.env.yaml").write_text("providers: {}")
        result = resolve_env_file(tmp_path, env=None)
        assert result == tmp_path / "vystak.env.yaml"

    def test_named_env(self, tmp_path):
        (tmp_path / "vystak.env.prod.yaml").write_text("providers: {}")
        result = resolve_env_file(tmp_path, env="prod")
        assert result == tmp_path / "vystak.env.prod.yaml"
...
class TestLoadBaseConfig:
    def test_loads_base_and_env(self, tmp_path):
        ...
        (tmp_path / "vystak.env.yaml").write_text(yaml.dump(env))
    ...
    def test_env_override(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "vystak.base.yaml").write_text(yaml.dump(base))
        (tmp_path / "vystak.env.prod.yaml").write_text(yaml.dump(prod))
        monkeypatch.setenv("VYSTAK_ENV", "prod")
        result = load_base_config(tmp_path)
        assert result["providers"]["azure"]["config"]["location"] == "westus2"

...
def generate_server_py(agent: Agent) -> str:
    ...
    lines.append('GATEWAY_URL = os.environ.get("AGENTSTACK_GATEWAY_URL", "")')
    lines.append("")
    lines.append("")
    lines.append("async def _register_with_gateway():")
    lines.append("    \"\"\"Self-register with the gateway if AGENTSTACK_GATEWAY_URL is set.\"\"\"")
    lines.append("    if not GATEWAY_URL:")
    lines.append("        return")
    lines.append("    import asyncio, httpx")
    lines.append("    agent_url = os.environ.get('AGENTSTACK_AGENT_URL', f'http://{HOST}:{PORT}')")
    lines.append("    for attempt in range(10):")
    lines.append("        try:")
    lines.append("            async with httpx.AsyncClient(timeout=10) as client:")
    lines.append("                resp = await client.post(f'{GATEWAY_URL}/register', json={")
    lines.append("                    'name': AGENT_NAME,")
    lines.append("                    'url': agent_url,")
    lines.append("                })")
    lines.append("                if resp.status_code == 200:")
    lines.append("                    print(f'Registered with gateway at {GATEWAY_URL}')")
    lines.append("                    return")
    lines.append("        except Exception:")
    lines.append("            pass")
    lines.append("        await asyncio.sleep(5)")
    lines.append("    print(f'Warning: could not register with gateway at {GATEWAY_URL}')")
    lines.append("")
    lines.append("")
    lines.append("async def _deregister_from_gateway():")
    lines.append("    \"\"\"Deregister from the gateway on shutdown.\"\"\"")
    lines.append("    if not GATEWAY_URL:")
    lines.append("        return")
    lines.append("    import httpx")
    lines.append("    try:")
    lines.append("        async with httpx.AsyncClient(timeout=5) as client:")
    lines.append("            await client.delete(f'{GATEWAY_URL}/unregister/{AGENT_NAME}')")
    lines.append("    except Exception:")
    lines.append("        pass")
    lines.append("")
    lines.append("")
    lines.append("@app.on_event('startup')")
    lines.append("async def _on_startup():")
    lines.append("    await _register_with_gateway()")
    lines.append("")
    lines.append("")
    lines.append("@app.on_event('shutdown')")
    lines.append("async def _on_shutdown():")
    lines.append("    await _deregister_from_gateway()")
    lines.append("")
    lines.append("")
    lines.append("class InvokeRequest(BaseModel):")
    lines.append("    message: str")
    lines.append("    session_id: str | None = None")
    lines.append("    trace_id: str | None = None")
    ...
    lines.append("    trace_id = request.trace_id or str(uuid.uuid4())")
    ...
    lines.append("        usage = {}")
    lines.append("        last_tool_call = None")
    lines.append(f"        async for chunk in {agent_ref}.astream(")
    lines.append('            {"messages": [("user", request.message)]},')
    lines.append('            config=config,')
    lines.append('            stream_mode=["messages", "custom"],')
    lines.append("        ):")
    lines.append('            # Custom events from tools (e.g., nested agent streaming)')
    lines.append('            if chunk[0] == "custom":')
    lines.append('                yield {"data": json.dumps({"type": "custom", "session_id": session_id, **chunk[1]})}')
    lines.append('            elif chunk[0] == "messages":')
    lines.append('                msg, metadata = chunk[1]')
    lines.append("                # AI message chunks — stream tokens")
    ...
    lines.append('                            yield {"data": json.dumps({"type": "token", "token": text, "session_id": session_id})}')
    lines.append("                    # Tool call chunks")
    lines.append("                    if msg.tool_call_chunks:")
    lines.append("                        for tc in msg.tool_call_chunks:")
    lines.append("                            if tc.get('name'):")
    lines.append('                                yield {"data": json.dumps({"type": "tool_call_start", "tool": tc["name"], "session_id": session_id})}')
    lines.append("                    # Usage from final AI message")
    ...
    lines.append("                # Tool result messages")
    ...
    lines.append('                    yield {"data": json.dumps({"type": "tool_result", "tool": tool_name, "result": output_str, "session_id": session_id})}')
    lines.append('        yield {"data": json.dumps({"type": "done", "done": True, "session_id": session_id, "usage": usage})}')
    lines.append("")

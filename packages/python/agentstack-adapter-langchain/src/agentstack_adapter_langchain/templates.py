...
def generate_server_py(agent: Agent) -> str:
    ...
    lines.append("        ):")
    lines.append('            etype = event["event"]')
    lines.append('            if etype == "on_chat_model_stream":')
    lines.append('                chunk = event["data"]["chunk"]')
    lines.append("                if chunk.content:")
    lines.append("                    if isinstance(chunk.content, str):")
    lines.append('                        yield {"data": json.dumps({"type": "token", "token": chunk.content, "session_id": session_id})}')
    lines.append("                    elif isinstance(chunk.content, list):")
    lines.append("                        for block in chunk.content:")
    lines.append("                            if isinstance(block, dict) and block.get('type') == 'text' and block.get('text'):")
    lines.append('                                yield {"data": json.dumps({"type": "token", "token": block["text"], "session_id": session_id})}')
    lines.append("                if chunk.tool_call_chunks:")
    lines.append("                    for tc in chunk.tool_call_chunks:")
    lines.append("                        if tc.get('name'):")
    lines.append('                            yield {"data": json.dumps({"type": "tool_call_start", "tool": tc["name"], "session_id": session_id})}')
    lines.append('            elif etype == "on_tool_end":')
    lines.append('                tool_name = event.get("name", "unknown")')
    lines.append('                output = event["data"].get("output", "")')
    lines.append("                output_str = str(output)[:200] if output else ''")
    lines.append('                yield {"data": json.dumps({"type": "tool_result", "tool": tool_name, "result": output_str, "session_id": session_id})}')
    lines.append('            elif etype == "on_chat_model_end":')

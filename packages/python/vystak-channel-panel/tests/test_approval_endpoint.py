...
class FakeResponsesClientHttp:
    ...
    get_checkpoint, resume_stream — scripted per call via queues so a test
    can drive one conversation through park -> approval -> resume."""
...
async def test_http_approval_resumes_and_persists_continuation(api, panel_rt, monkeypatch):
    owner, pid, cid = await _ready(api)
    fake = FakeResponsesClientHttp()
    panel_rt.responses_client = fake

...
def test_full_agent_definition():
    ...
    agent = ast.Agent(
        ...
        workspace=ast.Workspace(
            name="sandbox", type=ast.WorkspaceType.SANDBOX,
            filesystem=True, terminal=True, timeout="30m",
        ),
...
def test_hash_agent():
    anthropic = ast.Provider(name="anthropic", type="anthropic")
    model = ast.Model(name="sonnet", provider=anthropic, model_name="claude-sonnet-4-20250514")
    agent = ast.Agent(name="bot", model=model, skills=[ast.Skill(name="greeting", tools=["say_hello"])])

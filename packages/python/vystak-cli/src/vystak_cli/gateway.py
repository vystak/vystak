...
def inject_gateway_env(gateway_url: str, deployed: list[dict]) -> None:
    """Inject VYSTAK_GATEWAY_URL env var into deployed agents."""
    import os
    ...
    for d in deployed:
        ...
        try:
            ...
            rg_name = config.get("resource_group", f"vystak-{agent.name}-rg")
            ...
            if "VYSTAK_GATEWAY_URL" in existing_env:
                click.echo(f"  {agent.name}: already configured")
                continue

            # Add env var
            new_env = list(container.env or [])
            new_env.append(EnvironmentVar(name="VYSTAK_GATEWAY_URL", value=gateway_url))
...
def _resolve_agent_url(d: dict) -> str | None:
    ...
    if provider_type == "docker":
        ...
        return f"http://vystak-{agent.name}:8000"
...
def _deploy_gateway_docker(gateway_name: str) -> str | None:
    ...
    gateway_dir = Path(".vystak") / f"gateway-{gateway_name}"
...
def _deploy_gateway_azure(gateway_name: str, ref_agent) -> str | None:
    ...
    rg_name = config.get("resource_group", "vystak-gateway-rg")
    app_name = f"vystak-gateway-{gateway_name}"

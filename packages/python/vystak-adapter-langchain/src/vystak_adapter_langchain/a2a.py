...
def generate_a2a_handler_code(agent: Agent) -> str:
    ...
    lines.append("")
    lines.append('        # Persist any save_memory / forget_memory tool sentinels emitted')
    lines.append('        # during this turn. Mirrors the OpenAI-compatible chat-completions')
    lines.append('        # path; without it, A2A callers (e.g. Slack channel) lose memory writes.')
    lines.append("        if _store is not None:")

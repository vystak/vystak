"""Docker multi-agent + chat channel example.

Two agents (weather, time) + one chat channel routing to both. Point any
OpenAI-compatible client at http://localhost:18080 and switch agents via
the `model` field:

    model="vystak/weather-agent"  -> routed to the weather agent
    model="vystak/time-agent"     -> routed to the time agent

# docker-panel-nats — control panel over two agents (NATS transport)

Deploys two agents (weather, time) plus the `panel` channel: the control-panel
API container (users, projects, conversations, SSE streaming) over the NATS transport.

## NATS transport benefits

The deployment declares `transport: nats`, so the panel starts turns via `responses/createDetached`.
Streams flow through JetStream (`vystak-panel-nats-streams` stream on the `vystak-nats` broker),
...
The UI is the `vystak-panel` Next.js app (not deployed by `vystak apply`):
    ...
    cd ../../packages/typescript/vystak-panel   # from examples/docker-panel-nats
